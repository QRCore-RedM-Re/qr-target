...
CreateThread(function()
	local state = GetResourceState('qr-core')
	if state ~= 'missing' then
		local timeout = 0
		while state ~= 'started' and timeout <= 100 do
			timeout += 1
			state = GetResourceState('qr-core')
			Wait(0)
		end
		Config.Standalone = false
	end
	if Config.Standalone then
		...
		ItemCheck = QRCore.Functions.HasItem
		...
		RegisterNetEvent('QRCore:Client:OnGangUpdate', function(GangInfo)
