...
RegisterNetEvent('QRCore:Client:SetPlayerData')
AddEventHandler('QRCore:Client:SetPlayerData', function(val)
	PlayerData = val
end)
    ...
    CreateThread(function()
        ...
        elseif data.event ~= nil then
            ...
            elseif data.type == "qrcommand" then
                TriggerServerEvent('QRCore:CallCommand', data.event, data)
            else
                TriggerEvent(data.event, data)
            end
        else
            print("[qr-target]: ERROR NO EVENT SETUP")
        end
    end)
...
if Config.Debug then
	AddEventHandler('qr-target:debug', function(data)
